...
/* valor inicial, sino TEC1 y TEC2 no hacen nada hasta que se presione TEC3 y elija step... */
int step=15;                 //Pasos de angulos
...
/*
 * Pondría un valor inicial a step y actualizaría los leds en cambioAngulo
 */
...
/*
 * delayRead
 * 1° vez 	-> comienza a correr y devuelve 0
 * 2° vez 	-> si no pasó tiempo devuelve 0
 * 			-> si pasó tiempo, detiene y devuelve 1
 * 3° vez	-> comienza a correr y devuelve 0
 * etc...
 * Por eso se te ejecuta una vez la tecla y una vez no...
 */
				/*if ( !delayRead( &delay2 ) ){
					servoWrite( SERVO0, AnguloAsignado );
				}*/
